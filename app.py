...
storage_client = storage.Client.from_service_account_json('ventura-5d1fe-firebase-adminsdk-q6x4i-2a488de72f.json')
bucket_name = 'ventura-5d1fe.appspot.com'
...
# Function to get the list of user folders in Firebase Storage
...
# Function to delete a specific user's folder from Firebase Storage
def delete_user_folder(class_name):
    bucket = storage_client.bucket(bucket_name)
    blobs = bucket.list_blobs(prefix=f'ImagesAttendance/{class_name}/')
    for blob in blobs:
        blob.delete()
    return jsonify({'message': f'Folder "{class_name}" deleted successfully.'})
...
@app.route('/delete_user_folder/<string:class_name>', methods=['DELETE'])
def delete_user_folder_route(class_name):
    return delete_user_folder(class_name)
